...
class FeaturesOptions {
   ...
   fun connecttowifi(context:Context){
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
               context.openFileOutput("$filename", Context.MODE_PRIVATE).use { stream ->
                 ...
                 var fileuri= FileProvider.getUriForFile(context, "com.anmolsoftwaredeveloper12345.qrcodescanner.fileprovider", File(context.filesDir,"$filename"));
    ...
    fun sendemail(context: Context){
        try {
            val intent = Intent(Intent.ACTION_SENDTO)
            intent.data = Uri.parse("mailto:"+"anmolsoftwaredeveloper@gmail.com")
...
fun sharedPreferences(context: Context):Boolean{
    ...
    fun sharedPreferencesForRateUs(context: Context):Float{
        context.getSharedPreferences("RATE_US",
            Context.MODE_PRIVATE).let { sharedPreferences ->
            if(sharedPreferences.contains("rating")){
                return sharedPreferences.getFloat("rating",(0).toFloat())

            }else{
                sharedPreferences.edit().putFloat("rating",(0).toFloat())
                return (0).toFloat()
